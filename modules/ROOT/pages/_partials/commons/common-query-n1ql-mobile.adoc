...
Developers using Couchbase Lite for {param-title} can provide {sqlpp_cbm} query strings using the {sqlpp_cbm} Query API.
This API uses query statements of the form shown in <<ex-query-form>>.

The structure and semantics of the query format are based on that of Couchbase Server's {sqlpp_cbm} query language -- see {svr-n1ql-reference--xref} and {svr-n1ql-datamodel--xref}.


== Running

Use the N1QLQuery class to define the {sqlpp_cbm} query string on the database to be queried.
Then run the query using the`.execute()` method -- see {query-resultsets--xref} for how to work with result sets.

.Running a {sqlpp_cbm} Query
[#ex-samplerun]
:param-tags: query-syntax-n1ql
include::partial$block_tabbed_code_example.adoc[]
:param-tags!:
<.> Here we are accessing the current database using the shorthand notation *`_`* -- see the <<lbl-from>> clause for more on data source selection and <<lbl-query-params>> for more on parameterized queries.
...
Query Components::
<.> The <<lbl-select>> specifies the properties to be returned in the result set
<.> <<lbl-from>> specifies the data source to be queried
...
<.> <<lbl-limit>> specifies the maximum number of results to be returned
<.> <<lbl-offset>> specifies the number of results to be skipped before starting to return results
====

TIP: We recommend working through the {url-n1ql-tutorial} as a good way to build your {sqlpp_cbm} skills.
...
Each column is an expression which could be a property expression or any expression or function.
...
==== Select Wildcard
When using the `SELECT *` option the column name (key) of the {sqlpp_cbm} string is one of:

* The alias name if one was specified
* The data source name (or its alias if provided) as specified in the `FROM` clause.

This behavior is inline with that of Couchbase Server {sqlpp_cbm} -- see example in <<tbl-selstar>>.
...
|SELECT * FROM datasource AS store
|store
...
SELECT datasource.* AS data <.>

SELECT name fullName <.>

SELECT datasource.name fullName <.>
...
<.> Select all properties from the `datasource` data source. Give the object an alias name of `data`
<.> Select  pair of properties
<.> Select a specific property from the `datasource` data source.
...
[source, sql]
----
SELECT name FROM datasource
SELECT store.name FROM datasource AS store
SELECT store.name FROM datasource store
SELECT name FROM _
...
SELECT name FROM datasource WHERE department = ‘engineer’ AND group = ‘mobile
...
[source, sql]
----
SELECT name FROM datasource  ORDER BY name

SELECT name FROM datasource  ORDER BY name DESC

SELECT name, score FROM datasource  ORDER BY name ASC, score DESC
...
SELECT name FROM datasource LIMIT 10 <.>
...
SELECT name FROM datasource OFFSET 10 <.>

SELECT name FROM datasource  LIMIT 10 OFFSET 10 <.>
...
[source,sql]
----
SELECT value FROM datasource  WHERE value = true
SELECT value FROM datasource  WHERE value = false
----
...
[source,sql]
----
SELECT value FROM datasource  WHERE value = 10
SELECT value FROM datasource  WHERE value = 0
SELECT value FROM datasource WHERE value = -10
SELECT value FROM datasource WHERE value = 10.25
SELECT value FROM datasource WHERE value = 10.25e2
SELECT value FROM datasource WHERE value = 10.25E2
SELECT value FROM datasource WHERE value = 10.25E+2
SELECT value FROM datasource WHERE value = 10.25E-2
----
...
[source,sql]
----
SELECT firstName, lastName FROM datasource WHERE middleName = “middle”
SELECT firstName, lastName FROM datasource WHERE middleName = ‘middle’
----
...
[source,sql]
----
SELECT firstName, lastName FROM datasource WHERE middleName IS NULL
...
[source,sql]
----
SELECT firstName, lastName FROM datasource WHERE middleName IS MISSING
----
...
[source,sql]
----
SELECT { ‘name’: ‘James’, ‘department’: 10 } FROM datasource
SELECT { ‘name’: ‘James’, ‘department’: dept } FROM datasource
SELECT { ‘name’: ‘James’, ‘phones’: [‘650-100-1000’, ‘650-100-2000’] } FROM datasource
----
...
SELECT datasource.contact.address.city
  FROM datasource
  WHERE contact.name = "daniel"
...
This differs from Server N1QL, which performs float division regardless. Use `DIV(x, y)` to force float division in CBL N1QL
...
TIP: Use TOBOOLEAN(expr) function to convert a value based on Server {sqlpp_cbm} boolean value rules,
--
...
[NOTE]
This differs from Server {sqlpp_cbm} in the following instances: +
...
NOTE: Collate is not supported by Server N1QL
...
[source, sql]
----
SELECT department FROM datasource WHERE (name = "fred") COLLATE UNICODE
...
[source, sql]
----
SELECT department FROM datasource WHERE (name = "fred")
COLLATE (UNICODE)
----
...
[source, sql]
----
SELECT department FROM datasource WHERE (name = "fred") COLLATE (UNICODE CASE)
...
[source, sql]
----
SELECT name FROM datasource ORDER BY name COLLATE (UNICODE DIACRITIC)
...
[source,sql]
----
SELECT CASE state WHEN ‘CA’ THEN ‘Local’ ELSE ‘Non-Local’ END FROM datasource
----
...
SELECT CASE WHEN shippedOn IS NOT NULL THEN ‘SHIPPED’ ELSE "NOT-SHIPPED" END FROM datasource
...
^*^ The behavior of the ROUND() function is different from Server {sqlpp_cbm} ROUND(), which rounds the midpoint values using _Rounding to Nearest Even_ convention.
...
|SELECT META() FROM datasource

SELECT META().id, META().sequence, META().deleted, META().expiration FROM datasource
...
Couchbase Lite {sqlpp_cbm} Query supports all QueryBuilder features, except _Predictive Query_ and _Index_. See <<tbl-qbldr-diffs>> for the features supported by {sqlpp_cbm} but not by QueryBuilder.
...
You can provide runtime parameters to your {sqlpp_cbm} query to make it more flexible.
